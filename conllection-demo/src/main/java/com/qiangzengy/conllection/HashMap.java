...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * 最大容量，如果任意一个带有参数的构造函数隐式指定*时使用的最大容量。
     * 必须是两个<= 1 << 30的幂
     */
    ...
    //当链表的节点数等于8，转换成红黑树
    /**
     *对于插入，默认使用的是链表节点，当同一个位置的索引新增后达到9个，如果此时数组的长度，
     * 大于等于64，则会触发链表节点转红黑树节点，而如果数组长度小于64，则不会触发链表转红黑树，
     * 而是会进行扩容，因为此时数据量还比较小；
     * 对于移除索引，当同一个索引位置的节点移除后，达到6个，并且该索引的位置的节点为红黑树节点，
     * 会触发红黑树转换成链表
     */
    ...
    //链表转为红黑树的条件：1。链表的节点数等于8 ；2。总节点数等于64
    ...
    static final int hash(Object key) {
        int h;
        /**
         * 先判断key是否为null，为null返回0，
         * 否则取key的hashCode，再右移16位
         * （扰动函数)
         */
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            //n就是当前数组的长度16
            //resize()这个方法返回的是一个长度为16的空的数组
            n = (tab = resize()).length;

        /**
         * 这里没有用%运算，而用的是&运算，为什么用&运算？
         * 取余(%)操作中如果除数是2的幂次则等价于与其除数减一的与(&)操作(也就是说
         * hash%length=hash&(length-1)的前提是length是2的n次方;)。并且采用
         * 二进制位操作&,相对于%能够提高运算效率,这就解释了HashMap的长度为什么
         * 是2的幂次方。
         *
         * &：两位同时为“1”，结果才为“1”，否则为0
         */
    ...
    final Node<K, V>[] resize() {
        ...
        int newCap, newThr = 0;
        if (oldCap > 0) {//第一次这里不会走
            if (oldCap >= MAXIMUM_CAPACITY) {
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            //（newCap = oldCap << 1）将16扩容为32
    ...
    Node<K, V> replacementNode(Node<K, V> p, HashMap.Node<K, V> next) {
